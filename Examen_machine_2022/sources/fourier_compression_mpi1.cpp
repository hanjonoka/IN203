...
std::complex<double>* discretTransformFourier( std::uint32_t width, std::uint32_t height, unsigned char const* pixels, std::uint32_t lignes_par_proc, int rank)
{
...
int main(int nargs, char* argv[])
{
    ...
    std::uint32_t lignes_par_proc = width/nbp;

    std::chrono::time_point<std::chrono::system_clock> startDTF = std::chrono::system_clock::now();
    std::complex<double>* fourier_part = discretTransformFourier( width, height, image, lignes_par_proc, rank);
