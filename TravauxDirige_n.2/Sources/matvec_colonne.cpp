...
// =====================================================================
template<class T>
std::vector<T> add_vects(std::vector<T> u, std::vector<T> v)
{
...
int main( int argc, char* argv[] )
{
    ...
    if(rank==0)
    {
        for(int n = 1; n<nbp; n++)
        {
            ...
            MPI_Status status;
            MPI_Recv(buf.data(), N, MPI_DOUBLE, n, 0, globComm, &status);
            v = add_vects(v,buf);
        }
    }else{
        MPI_Send(v.data(), N, MPI_DOUBLE, 0, 0, globComm);
    }

    MPI_Bcast(v.data(), N, MPI_DOUBLE, 0, globComm);
